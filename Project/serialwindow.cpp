    ...
    : QWidget(parent), ui(new Ui::serialwindow), timer(new QTimer(this))
{
    ...
    lightLabel->setPixmap(QPixmap("../Project/images/GrapLED.png"));       // 设置初始状态为灭灯（使用加载成功的pixmap）
    /*初始化图表*/
    charts_init();
    // 设置定时器的时间间隔为1ms
    timer->setInterval(1);
...
void serialwindow::led_blink(std::vector<unsigned char>& input)
{
    ...
    {
        _state = 0;
        lightLabel->setPixmap(QPixmap("./images/GreenLED.png")); // 设置初始状态为灭灯
    }else{
        _state = 1;
        lightLabel->setPixmap(QPixmap("./images/GrapLED.png")); // 设置初始状态为灭灯
    }
}
